...
export const AUDIT_LOG_FILTER_MODULES = Object.keys(AUDIT_LOG_MODULE_LABELS);
...
export const getAuditLogTargetTypeLabel = (value) => AUDIT_LOG_TARGET_TYPE_LABELS[value] ?? value;
...
export const formatAuditTarget = (target = {}) => {
  ...
  const targetTypeLabel = getAuditLogTargetTypeLabel(targetType);
  ...
  if (!hasValue(targetType)) {
    return hasDisplayId(targetId) ? `#${targetId}` : '-';
  }

  return hasDisplayId(targetId) ? `${targetTypeLabel} #${targetId}` : targetTypeLabel;
};
