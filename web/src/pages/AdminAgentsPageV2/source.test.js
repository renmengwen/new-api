...
test('admin agents page loads assignable token groups for agent forms', () => {
  ...
  assert.ok(
    pageSource.includes('toManagedGroupOptions(res.data.data, userGroup)'),
  );
});
...
test('admin agents page submits allowed token groups alongside the primary group', () => {
  ...
  assert.ok(
    pageSource.includes('allowed_token_groups: normalizedAllowedTokenGroups'),
  );
...
test('admin agents page shows allowed token group controls without removing submit logic', () => {
  ...
  assert.ok(
    pageSource.includes("<Text type='tertiary'>{t('限制令牌分组')}</Text>"),
  );
  assert.ok(
    pageSource.includes("<Text type='tertiary'>{t('可创建令牌分组')}</Text>"),
  );
});

test('admin agents page submits a required email address', () => {
  assert.ok(pageSource.includes("email: '',"));
  assert.ok(pageSource.includes('!formState.email.trim()'));
  assert.ok(pageSource.includes('email: formState.email.trim()'));
  assert.ok(
    pageSource.includes("<Text type='tertiary'>{t('邮箱地址')}</Text>"),
  );
  assert.ok(pageSource.includes("placeholder={t('请输入邮箱地址')}"));
});
