...
func buildTestRequest(model string, endpointType string, channel *model.Channel, isStream bool) dto.Request {
	...
	if strings.Contains(strings.ToLower(model), "codex") || (channel != nil && channel.Type == constant.ChannelTypeVolcEngine && common.IsVolcEngineResponsesModel(resolveChannelTestModelName(channel, model))) {
		return &dto.OpenAIResponsesRequest{
			Model:  model,
			Input:  testResponsesInput,
			Stream: lo.ToPtr(isStream),
